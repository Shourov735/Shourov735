<div align="center">
  <img src="https://raw.githubusercontent.com/Shourov735/Shourov735/main/assets/banner.svg" width="100%" alt="Md. Shourov banner"/>
</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&pause=1000&color=6366F1&center=true&vCenter=true&width=720&lines=Software+Engineering+Student+at+IIT%2C+University+of+Dhaka;Full-Stack+Web+%26+Mobile+Application+Developer;IT+Secretary+%40+Nabodigonto+Welfare+Organization;Open+Source+Contributor+%26+Design+Pattern+Practitioner)](https://github.com/Shourov735)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/md-shourov-89125a337)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Shourov735)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=flat-square&logo=googlechrome&logoColor=white)](https://shourov735.vercel.app/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:mdshourov735@gmail.com)
[![X / Twitter](https://img.shields.io/badge/X-000000?style=flat-square&logo=x&logoColor=white)](https://x.com/Shourov735)
[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=flat-square&logo=telegram&logoColor=white)](https://t.me/Shourov735)
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=flat-square&logo=youtube&logoColor=white)](https://www.youtube.com/@mdshourovgaming)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=flat-square&logo=medium&logoColor=white)](https://medium.com/@Shourov735)

</div>

---

## 👨‍💻 About Me

I am a Software Engineering undergraduate at the **Institute of Information Technology (IIT), University of Dhaka**. I specialize in architecting full-stack web applications, offline-first mobile apps, and robust desktop systems with strong emphasis on software architecture, Gang of Four (GoF) design patterns, and clean code principles.

- 🎓 **Education:** B.Sc. in Software Engineering, Institute of Information Technology (IIT), University of Dhaka
- 📍 **Location:** Gazipur & Dhaka, Bangladesh
- 💼 **Leadership:** **IT Secretary** at *Nabodigonto Social Welfare Organization* (promoted after designing and launching their digital community and financial management platform)
- 🌐 **Portfolio & Contact:** [shourov735.vercel.app](https://shourov735.vercel.app/) • [mdshourov735@gmail.com](mailto:mdshourov735@gmail.com)
- 🚀 **Focus Areas:** Enterprise Web Apps (Next.js, Cloudflare Workers, Neon PostgreSQL), Mobile Solutions (React Native, Expo), System Architecture (Design Patterns, SOLID), Open Source Contributions

---

## 🚀 Featured Projects

### 🌟 1. [Nabodigonto (নবদিগন্ত সমাজ কল্যাণ সংস্থা)](https://nabodigonto.conversora-tech.workers.dev/) — Flagship Community & Financial Platform

[![Live Demo](https://img.shields.io/badge/Live_Demo-nabodigonto.conversora--tech.workers.dev-00e599?style=flat-square&logo=cloudflare&logoColor=white)](https://nabodigonto.conversora-tech.workers.dev/)
[![Role](https://img.shields.io/badge/Role-IT_Secretary-6366f1?style=flat-square)](#)
[![Stack](https://img.shields.io/badge/Stack-Next.js_16_%7C_Prisma_7_%7C_Neon_Postgres_%7C_Clerk_v7_%7C_Cloudflare-black?style=flat-square)](#)

> **Impact:** Following the successful architecture, implementation, and deployment of this platform, I was appointed as the **IT Secretary** of Nabodigonto Social Welfare Organization (*নবদিগন্ত সমাজ কল্যাণ সংস্থা*), Gazipur.

A comprehensive digital platform engineered for a grassroots Bangladeshi social welfare organization:
- **Blind Audit Protocol & Double-Signed Verification:** To prevent financial discrepancies, expected treasury balances remain masked until both the Finance Director and System Admin independently submit physical cash counts and bank statements.
- **Dynamic 3D Flip Member ID Cards:** Generates digital PVC-style identity cards with dynamic QR codes for public verification (`/verify/[token]`) while securing sensitive member details. Includes bulk PVC print-ready layout generation (`/admin/cards/bulk`).
- **Member Dues & Transparency Desk:** Self-service member hub for tracking monthly dues, instant receipts, and downloading cryptographically verified SHA-256 PDF financial audit reports.
- **Modern Cloud Architecture:** Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS v4, Prisma 7, Neon Serverless PostgreSQL / Cloudflare D1, Clerk Auth v7, Backblaze B2 (S3-compatible) storage, deployed serverless on Cloudflare Workers via OpenNext.

---

### 📚 2. [InsideJibon](https://insidejibon.insidejibon.workers.dev/) — Next-Gen EdTech Web Platform

[![Live Demo](https://img.shields.io/badge/Live_Demo-insidejibon.insidejibon.workers.dev-f38020?style=flat-square&logo=cloudflare&logoColor=white)](https://insidejibon.insidejibon.workers.dev/)
[![Repository](https://img.shields.io/badge/Repo-InsideJibon-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Shourov735/InsideJibon)
[![Stack](https://img.shields.io/badge/Stack-Next.js_16_%7C_Drizzle_ORM_%7C_Neon_Postgres_%7C_R2-black?style=flat-square)](#)

A scalable educational web platform built for friend Tanvir Hasan Jibon to power modern course management and digital learning:
- **Full Learning Lifecycle:** Comprehensive domain modules covering course catalogs, student enrollments, exam creation, assignments, and automated grading.
- **Bilingual Experience:** Built-in internationalization (en/bn) parity validation across course materials and administrative portals.
- **Edge Performance & Cloud Storage:** Built with Next.js 16, React 19, Tailwind CSS v4, Drizzle ORM on Neon Serverless PostgreSQL, Cloudflare R2 object storage for lecture resources, deployed globally on Cloudflare Workers.

---

### 🎯 3. [Quiz Management & Examination System](https://github.com/Shourov735/Quiz-Management-and-Examination-System) — Mini-SPL / Design Patterns Project

[![Repository](https://img.shields.io/badge/Repo-Quiz--Management--System-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Shourov735/Quiz-Management-and-Examination-System)
[![Academic Project](https://img.shields.io/badge/Academic_Project-Mini--SPL_%2F_Design_Patterns-blue?style=flat-square)](#)
[![Stack](https://img.shields.io/badge/Stack-Java_%7C_JavaFX_%7C_SQLite_%7C_GoF_Patterns-ed8b00?style=flat-square&logo=openjdk&logoColor=white)](#)

A desktop assessment system engineered in Java and JavaFX for the **Design Patterns Lab Final Project (Mini-SPL)** at IIT, University of Dhaka:
- **Rigorous GoF Design Patterns & SOLID:** Architected using Factory, Builder (custom exam composition), Strategy (scoring & grading schemes), Observer (event-driven exam timers & submission triggers), Command, and Repository patterns.
- **Dual Role Interfaces:** Separate dashboards for **Teachers** (question authoring, category management, examination scheduling, analytical reports) and **Students** (timed quiz engine, anti-cheat screen management, instant performance review).
- **Persistence & Quality Assurance:** Relational persistence with SQLite, comprehensive database seeders, validation pipelines, and JUnit 5 test suites.

---

### 📖 4. [Quran Reading Tracker](https://github.com/Shourov735/QuranReadingTracker) — Offline-First Mobile App

[![Repository](https://img.shields.io/badge/Repo-QuranReadingTracker-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Shourov735/QuranReadingTracker)
[![Platform](https://img.shields.io/badge/Platform-Android_%2F_Offline--First-3DDC84?style=flat-square&logo=android&logoColor=white)](#)
[![Stack](https://img.shields.io/badge/Stack-React_Native_%7C_Expo_%7C_TypeScript-61dafb?style=flat-square&logo=react&logoColor=black)](#)

An offline-first Android application designed to cultivate and track daily Quran reading habits with total data sovereignty:
- **Dual Independent Reading Pipelines:** Tracks Arabic recitation (by Surah + Ruku) and Bangla translation reading (by Surah + Ayat) separately so differing habit paces never interfere.
- **Privacy by Design:** Zero third-party trackers, zero cloud dependencies, and zero accounts. All logs, streaks, and progress analytics persist locally via AsyncStorage.
- **Rich Experience:** Clean dark-mode UI, streak calculators, progress charts, and seamless APK builds via Expo EAS.

---

### 🕌 5. [Mosque Library Management](https://mosque-library-management.vercel.app/) — Modern Web App

[![Live Demo](https://img.shields.io/badge/Live_Demo-Mosque_Library-0070f3?style=flat-square&logo=vercel&logoColor=white)](https://mosque-library-management.vercel.app/)
[![Stack](https://img.shields.io/badge/Stack-Next.js_%7C_Prisma_%7C_PostgreSQL_%7C_Clerk-black?style=flat-square)](#)

A specialized library management system tailored for community mosques:
- Facilitates cataloging of Islamic literature, member borrowings, loan return tracking, and inventory auditing.
- Powered by Next.js, Prisma ORM, PostgreSQL, and Clerk authentication.

---

### 🕹️ 6. [TetriC](https://github.com/Shourov735/TetriC) — Modular Multiplayer C Game

[![Repository](https://img.shields.io/badge/Repo-TetriC-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Shourov735/TetriC)
[![Type](https://img.shields.io/badge/Type-Socket_Programming_%7C_C-A8B9CC?style=flat-square&logo=c&logoColor=white)](#)

- Modular, console-based Tetris game built in C for a university Software Engineering project.
- Features single-player, local 2-player, and socket-based network multiplayer.
- Implements collision detection, matrix rotation algorithms, scoring systems, level progression, and persistent high-score files.

---

## 🌐 Open Source Contributions

### 📦 [romankh3/image-comparison](https://github.com/romankh3/image-comparison) (Maven Central Java Library)
[![Pull Request](https://img.shields.io/badge/PR_%23255-Refactor_Region_Grouping-238636?style=flat-square&logo=github&logoColor=white)](https://github.com/romankh3/image-comparison/pull/255)
[![Upstream](https://img.shields.io/badge/Upstream-romankh3%2Fimage--comparison-blue?style=flat-square&logo=github&logoColor=white)](https://github.com/romankh3/image-comparison)
[![Assignment](https://img.shields.io/badge/Project-University_Open_Source_Assignment-8a2be2?style=flat-square)](#)

Contributed to this widely used Java QA visual testing library as part of an academic Open Source Software Engineering assignment at University of Dhaka:
- **Algorithmic Refactoring ([PR #255](https://github.com/romankh3/image-comparison/pull/255) / Closes [#254](https://github.com/romankh3/image-comparison/issues/254)):**
  - Replaced recursive difference region grouping with an iterative `ArrayDeque` traversal, protecting against stack overflow errors during large connected image diffs.
  - Replaced per-pixel `new Point(x, y)` object instantiations during exclusion checks with coordinate-based checks, dramatically reducing memory overhead and GC pressure.
  - Eliminated transient comparison state stored on the class instance, refactored rectangle merging to "merge until stable", and added regression tests.
  - Passed 100% of the test suite (66/66 JUnit tests), PMD code quality checks, and CPD duplication checks.

---

## 🛠️ Tools I Reach For

### 💻 Programming Languages
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=c,cpp,java,python,ts,js,html,css,bash&theme=dark" alt="Programming Languages" />
  </a>
</p>

### 🎨 Frontend & Mobile Development
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,flutter&theme=dark" alt="Frontend and Mobile" />
  </a>
</p>

### 🗄️ Backend, ORMs & Databases
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=nodejs,prisma,postgres,mysql,sqlite,firebase&theme=dark" alt="Backend and Databases" />
  </a>
</p>

### ☁️ Cloud, DevOps & Tools
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=cloudflare,vercel,docker,git,github,githubactions,linux,vscode,postman,maven&theme=dark" alt="Cloud and DevOps" />
  </a>
</p>

---

## 🗣️ Spoken Languages

- 🇧🇩 **Bangla (বাংলা):** Native
- 🇬🇧 **English:** Professional Working Proficiency

---

## 📊 GitHub Analytics

<div align="center">
  <a href="https://github.com/Shourov735">
    <img height="165" src="https://github-readme-stats-fast.vercel.app/api?username=Shourov735&show_icons=true&theme=github_dark&include_all_commits=true&count_private=true&hide_border=true&border_radius=8" alt="Md. Shourov GitHub Stats" />
  </a>
  <a href="https://github.com/Shourov735">
    <img height="165" src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=Shourov735&layout=compact&langs_count=8&theme=github_dark&hide_border=true&border_radius=8" alt="Md. Shourov Top Languages" />
  </a>
</div>

<br/>

<div align="center">
  <a href="https://github.com/Shourov735">
    <img src="https://streak-stats.demolab.com?user=Shourov735&theme=github_dark&hide_border=true&border_radius=8" alt="Md. Shourov GitHub Streak" />
  </a>
</div>

### 📈 Contribution Activity

<div align="center">
  <a href="https://github.com/Shourov735">
    <img width="100%" src="https://github-activity-graph.vercel.app/graph?username=Shourov735&theme=github-dark&hide_border=true&area=true" alt="Md. Shourov Activity Graph" />
  </a>
</div>

<br/>

<div align="center">
  <a href="https://github.com/Shourov735">
    <img width="100%" src="https://ghchart.rshah.org/6366f1/Shourov735" alt="Md. Shourov GitHub Contribution Chart" />
  </a>
</div>

---

<div align="center">

> *"Learning by building, improving one project at a time."*

**Let's build something remarkable together!**  
[Portfolio](https://shourov735.vercel.app/) • [LinkedIn](https://linkedin.com/in/md-shourov-89125a337) • [Email](mailto:mdshourov735@gmail.com)

</div>